...
#Cette grille contient des obstacles
...
class EnvironnementGrille2DObs(Environment):
    ...
    def __init__(self,params):
        ...
        #Liste des obstacles: un obstacle occupe un état= (ligne-1)*c+colonne
        self.obstacle=[(int(8*l/10)-1)*c+1, (int(8*l/10)-1)*c+2, (int(8*l/10)-1)*c+3, (int(l/3)-1)*c+int(c/2), ((1+int(l/3))-1)*c+int(c/2), ((2+int(l/3))-1)*c+int(c/2),((3+int(l/3))-1)*c+int(c/2), (int(l/2)-1)*c+int(c/2), (int(l/2)-1)*c+int(c/2)+1, (int(l/2)-1)*c+int(c/2)+2]
    ...
    def step(self, action):
        ...
        #Variable qui indique si l'agent s'est cogné ou non
        cogne=False
